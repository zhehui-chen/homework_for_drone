...
int main(int argc, char **argv)
{
  ...
  while (ros::ok()){
    ...
    //give the linear velocity to the turtle
    if (sqrt(pow(pos_err.x,2)+pow(pos_err.y,2)) > 0.05){
        vel_msg.linear.x = 0.5;
        printf("pos_err : %f\n",sqrt(pow(pos_err.x,2)+pow(pos_err.y,2)));
    }else{
        vel_msg.linear.x = 0;
    }

    //give the angular velocity to the turtle
    if (pos_err.theta > 0.05){
        vel_msg.angular.z = 0.5;
        printf("pos_err.theta : %f\n",pos_err.theta);
